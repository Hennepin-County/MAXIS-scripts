...
Dialog1 = ""
BeginDialog Dialog1, 0, 0, 351, 90, "Start Resolve HC EOMC in MMIS Run"
  ...
  CheckBox 10, 75, 200, 10, "Check here to restart AFTER the Full report is done.", restart_checkbox
...
If restart_checkbox <> checked Then
...
Else
	...
	curr_yr = DatePart("yyyy", date)
	file_friendly_date = curr_mo & "-" & curr_day & "-" & right(curr_yr, 2)
	EOMC_file_name = t_drive & "\Eligibility Support\Restricted\QI - Quality Improvement\BZ scripts project\Projects\HC Discrepancy\EOMC\" & CM_plus_1_mo & "-20" & CM_plus_1_yr & " Change Run - " & file_friendly_date & ".xlsx"
	call excel_open(EOMC_file_name, True, True, ObjExcel, objWorkbook)

	email_subject = "Resolve HC EOMC in MMIS Script Run Completed"

	email_body = "The script run to align MMIS to MAXIS EOMC for HC has completed."
	email_body = email_body & vbCr & vbCr & "Script Run Stats:"
	email_body = email_body & vbCr & "-  Query Runtime: " &  ObjExcel.Cells(2, 22).Value
	email_body = email_body & vbCr & "-  Total Savings: $ " &  ObjExcel.Cells(3, 22).Value
	email_body = email_body & vbCr & "-  Total PMIs: " &  ObjExcel.Cells(4, 22).Value
	email_body = email_body & vbCr & "-  Manually Closed PMIs: " &  ObjExcel.Cells(5, 22).Value
	email_body = email_body & vbCr & "-  MMIS Span Not Updated: " &  ObjExcel.Cells(6, 22).Value
	email_body = email_body & vbCr & "-  PMIs with savings: " &  ObjExcel.Cells(7, 22).Value
	email_body = email_body & vbCr & "-  PMIs Updated MMIS by script: " &  ObjExcel.Cells(8, 22).Value
	email_body = email_body & vbCr & vbCr & "Report Link: <" & file_url & ">"
	email_body = email_body & vbCr & vbCr & "SCRIPT GENERATED REPORT OUT"
	...
	Do
		...
		EOMC_CLIENT_ARRAY(basket_nbr,   hc_clt) = ObjExcel.Cells(excel_row, 1).Value
		EOMC_CLIENT_ARRAY(case_nbr,  hc_clt) = ObjExcel.Cells(excel_row, 2).Value
		EOMC_CLIENT_ARRAY(hc_close_stat, hc_clt) = ObjExcel.Cells(excel_row, 3).Value
		EOMC_CLIENT_ARRAY(clt_ref_nbr, hc_clt) = ObjExcel.Cells(excel_row, 4).Value
		EOMC_CLIENT_ARRAY(clt_ref_nbr, hc_clt) = replace(EOMC_CLIENT_ARRAY(clt_ref_nbr, hc_clt), "Memb ", "")
		EOMC_CLIENT_ARRAY(clt_name,  hc_clt) = ObjExcel.Cells(excel_row, 5).Value
		EOMC_CLIENT_ARRAY(clt_pmi,   hc_clt) = ObjExcel.Cells(excel_row, 6).Value
		EOMC_CLIENT_ARRAY(hc_prog_one,   hc_clt) = ObjExcel.Cells(excel_row, 7).Value
		EOMC_CLIENT_ARRAY(elig_type_one,   hc_clt) = ObjExcel.Cells(excel_row, 8).Value
		EOMC_CLIENT_ARRAY(prog_one_end, hc_clt) = ObjExcel.Cells(excel_row, 9).Value
		EOMC_CLIENT_ARRAY(MMIS_curr_end_one, hc_clt) = ObjExcel.Cells(excel_row, 10).Value

		EOMC_CLIENT_ARRAY(hc_prog_two,   hc_clt) = ObjExcel.Cells(excel_row, 13).Value
		EOMC_CLIENT_ARRAY(elig_type_two,   hc_clt) = ObjExcel.Cells(excel_row, 14).Value
		EOMC_CLIENT_ARRAY(prog_two_end, hc_clt) = ObjExcel.Cells(excel_row, 15).Value
		EOMC_CLIENT_ARRAY(MMIS_curr_end_two, hc_clt) = ObjExcel.Cells(excel_row, 16).Value
		EOMC_CLIENT_ARRAY(err_notes, hc_clt) = ObjExcel.Cells(excel_row, 19).Value

		If make_changes = TRUE Then
			EOMC_CLIENT_ARRAY(MMIS_new_end_one, hc_clt) = ObjExcel.Cells(excel_row, 11).Value
			Call read_boolean_from_excel(ObjExcel.Cells(excel_row, 12).Value, EOMC_CLIENT_ARRAY(capitation_ended, hc_clt))
			' EOMC_CLIENT_ARRAY(capitation_ended, hc_clt) = ObjExcel.Cells(excel_row, cap_col).Value
			EOMC_CLIENT_ARRAY(MMIS_new_end_two, hc_clt) = ObjExcel.Cells(excel_row, 17).Value
			EOMC_CLIENT_ARRAY(clt_savings, hc_clt) = ObjExcel.Cells(excel_row, 18).Value
			' ObjExcel.Cells(excel_row, savings_col).NumberFormat = "$#,##0.00"
		End If
...
If make_changes = TRUE Then
    ...
    Do
        ...
        For i = 1 to col_to_use
            ObjExcel.columns(col_to_use).AutoFit()
        Next
