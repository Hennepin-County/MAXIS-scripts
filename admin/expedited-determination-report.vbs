...
this_month_worklist = MonthName(DatePart("m", date)) & " " & DatePart("yyyy", date)

ObjHSSExcel.worksheets(this_month_worklist).Activate
...
For Each objFile in colFiles																'looping through each file
	...
	If DateDiff("d", this_file_created_date, date) > 0 Then
		...
		If exp_det = True Then
			...
			If row <> 0 Then
				...
				If fs_status = "PENDING" Then report_to_HSS = True
			End If
			Call back_to_SELF
			MAXIS_case_number = ""
		End If
			...
			For Each text_line in exp_det_details										'read each line in the file
				If Instr(text_line, "^*^*^") <> 0 Then
					...
					If line_info(0) = "WORKER NAME" Then
						...
						For each_wrkr = 0 to UBound(WORKER_ARRAY, 2)
