		...
		If InStr(list_of_all_cases, "*" & case_number_to_assess & "*") = 0 Then 		'making sure we don't have repeat case numbers
			...
			For each_case = 0 to UBound(TODAYS_CASES_ARRAY, 2)              'This loops through each case that was on the BOBI today
		        ...
		        If case_name_to_assess = TODAYS_CASES_ARRAY(client_name, each_case) Then  'If a matching case number is found this means the case was on the working excel AND is on the BOBI
					...
					yesterdays_action_info = UCase(ALL_PENDING_CASES_ARRAY(script_notes_info, case_entry))
					If InStr(yesterdays_action_info, "FOLLOW UP NEEDED") <> 0 Then
						ALL_PENDING_CASES_ARRAY(add_to_daily_worklist, case_entry) = True
						'TODO add reading of the STS messages and functionality
						ALL_PENDING_CASES_ARRAY(script_notes_info, case_entry) = Replace(ALL_PENDING_CASES_ARRAY(script_notes_info, case_entry), "FOLLOW UP NEEDED", "")
						ALL_PENDING_CASES_ARRAY(script_notes_info, case_entry) = trim(ALL_PENDING_CASES_ARRAY(script_notes_info, case_entry))
						If InStr(ALL_PENDING_CASES_ARRAY(script_notes_info, case_entry), "Carry over from Yesterday Worklist") = 0 Then ALL_PENDING_CASES_ARRAY(script_notes_info, case_entry) = "Carry over from Yesterday Worklist. " & ALL_PENDING_CASES_ARRAY(script_notes_info, case_entry)
						ALL_PENDING_CASES_ARRAY(script_notes_info, case_entry) = trim(ALL_PENDING_CASES_ARRAY(script_notes_info, case_entry))
					End If
					If InStr(ALL_PENDING_CASES_ARRAY(script_notes_info, case_entry), "STS-NR") <> 0 Then
						ALL_PENDING_CASES_ARRAY(add_to_daily_worklist, case_entry) = True
						ALL_PENDING_CASES_ARRAY(script_notes_info, case_entry) = replace(ALL_PENDING_CASES_ARRAY(script_notes_info, case_entry), "STS-NR", "")
						If InStr(ALL_PENDING_CASES_ARRAY(script_notes_info, case_entry), "Carry over from Yesterday Worklist") = 0 Then ALL_PENDING_CASES_ARRAY(script_notes_info, case_entry) = "Carry over from Yesterday Worklist. " & ALL_PENDING_CASES_ARRAY(script_notes_info, case_entry)
					End If
...
Do While NOT objWorkRecordSet.Eof
	...
	If InStr(list_of_all_cases, "*" & case_number_to_assess & "*") = 0 and case_number_to_assess <> "2536665" Then 		'making sure we don't have repeat case numbers
		...
		If WORKING_LIST_CASES_ARRAY(next_action_needed, case_entry) = "ALIGN INTERVIEW DATES" Then WORKING_LIST_CASES_ARRAY(next_action_needed, case_entry) = ""
...
For case_entry = 0 to UBOUND(WORKING_LIST_CASES_ARRAY, 2)
	...
	IF WORKING_LIST_CASES_ARRAY(next_action_needed, case_entry) = "ALIGN INTERVIEW DATES" Then
		If WORKING_LIST_CASES_ARRAY(align_intv_revw_dt, case_entry) = "" or WORKING_LIST_CASES_ARRAY(align_intv_revw_dt, case_entry) = False Then
			WORKING_LIST_CASES_ARRAY(add_to_daily_worklist, case_entry) = True
		ElseIf IsDate(WORKING_LIST_CASES_ARRAY(align_intv_revw_dt, case_entry)) = True Then
			If DateDiff("d", WORKING_LIST_CASES_ARRAY(align_intv_revw_dt, case_entry) = False, date) > 7 Then WORKING_LIST_CASES_ARRAY(add_to_daily_worklist, case_entry) = True
		End If
	End If
