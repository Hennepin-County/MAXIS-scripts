...
function assign_a_case()
    ...
    Do While NOT objRecordSet.Eof    'Read the whole table
        ...
        If Instr(case_tracking_notes, "STS-NR") <> 0 Then
            ...
            assigned_appt_date_confirmation         = objRecordSet("Confirmation")
    ...
    objRecordSet.Open "UPDATE ES.ES_OnDemanCashAndSnapBZProcessed SET CaseNumber = '" & MAXIS_case_number & "', " &_
                                                                      ...
                                                                      "Confirmation = '" & assigned_appt_date_confirmation & "', " &_
    ...
    Do
        ...
        BeginDialog Dialog1, 0, 0, 246, 335, "On Demand Applications Case Review"
          ...
          Text 30, 45, 210, 10, "Case Name: THIIS IS A LONG NAME WITH TOO MANY IIIIIS" & assigned_case_name
          Text 15, 60, 120, 10, "Application Date: MM/DD/YY" & assigned_application_date
          Text 20, 70, 75, 10, " Days Pending: XX" & assigned_rept_pnd2_days
          Text 45, 80, 80, 10, "Day 30: MM/DD/YY" & assigned_day_30
          ...
          Text 20, 115, 140, 10, "Interview Date from PROG: MM/DD/YY" & assigned_interview_date
          ...
          If assigned_next_action_needed = "RESOLVE SUBSEQUENT APPLICATION DATE" Then Text 20, 235, 145, 10, "Subsequent Appliction Date: MM/DD/YY" & assigned_2nd_application_date
...
For tester = 0 to UBound(tester_array)                         'looping through all of the testers
    ...
    End If
Next
If qi_member_identified = False Then script_end_procedure("This script can only be operated by a member of QI due to access restrictions. The script will now end.")
...
If BULK_Run_completed = False Then
    Do
        Do
            ...
            If ButtonPressed = complete_bulk_run_btn Then Call run_from_GitHub(script_repository & "admin\" & "on-demand-waiver-applications.vbs")
        Loop
        CALL check_for_password(are_we_passworded_out)			'function that checks to ensure that the user has not passworded out of MAXIS, allows user to password back into MAXIS
    Loop until are_we_passworded_out = false					'loops until user passwords back in
End If
	    ...
	    Do
	        ...
	        BeginDialog Dialog1, 0, 0, 451, 310, "On Demand Applications Case Review"
	          ...
	          If assigned_next_action_needed = "REVIEW RECENT CLOSURE/DENIAL" Then
	        	  Text 260, 110, 145, 10, "Case closed in past 30 Days"
	        	  Text 260, 120, 50, 10, "Review Notes:"
	        	  EditBox 260, 130, 175, 15, assigned_closed_recently_resolve
	          End If
	          If assigned_next_action_needed = "REVIEW OTHER COUNTY CASE" Then
	        	  Text 260, 155, 145, 10, "Case was in Another County"
	        	  Text 260, 165, 50, 10, "Review Notes:"
	        	  EditBox 260, 175, 175, 15, assigned_out_of_county_resolve
	          End If
	          If left(assigned_next_action_needed, 18) = "REVIEW CANNOT DENY" Then
	        	  Text 260, 200, 175, 10, "Case Cannot be Denied"
	        	  ' Text 260, 210, 175, 10, "REASON"
	        	  If assigned_next_action_needed = "REVIEW CANNOT DENY - No Appt Notc" Then Text 260, 210, 175, 10, "No Appointment Notice"
	        	  If assigned_next_action_needed = "REVIEW CANNOT DENY - No NOMI" Then Text 260, 210, 175, 10, "No NOMI"
	        	  If assigned_next_action_needed = "REVIEW CANNOT DENY - NOMI after Day 30" Then Text 260, 210, 175, 10, "NOMI after Day 30"
	        	  Text 260, 220, 50, 10, "Resolution:"
	        	  EditBox 260, 230, 175, 15, cannot_deny_resolution
	          End If
	          Text 10, 260, 70, 10, "Additional Notes:"
