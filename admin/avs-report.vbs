...
BeginDialog initial_dialog, 0, 0, 246, 95, "AVS Processing Selection"
  DropListBox 120, 50, 115, 12, "Select one..."+chr(9)+"Initial Monthly Upload"+chr(9)+"ECF Forms Received"+chr(9)+"Person and Case Noting Forms"+chr(9)+"Initial Memo"+chr(9)+"Secondary Memo", AVS_option
...
If AVS_option = "ECF Forms Received" then 
    ...
    msgbox "Number of forms reviewed:" & entry_record & vbcr & "Number of forms found:" & form_count    
...
If AVS_option = "Person and Case Noting Forms" then 
    DIM note_array()
    ReDim note_array(4, 0)
    
    'constants for array
    const MAXIS_case_number_const   = 0
    const client_PMI_const 	        = 1
    const client_name_const         = 2
    const form_date_const           = 3
    const note_date_const           = 4
    ...
    Do   
        ...
        ReDim Preserve note_array(4, entry_record)	'This resizes the array based on the number of rows in the Excel File'
        note_array(MAXIS_case_number_const, entry_record) = MAXIS_case_number	'The client information is added to the array'
        note_array(client_PMI_const,        entry_record) = client_PMI		
	    note_array(client_name_const,       entry_record) = client_name  
        note_array(form_date_const,         entry_record) = form_date                    
        note_array(note_date_const,	        entry_record) = note_date
    ...
    excel_row = 2
    For item = 0 to UBound(note_array, 2)
        MAXIS_case_number =     note_array(MAXIS_case_number_const, item)
        Client_PMI =            note_array(client_PMI_const, item)
        client_name =           note_array(client_name_const, item)
        form_date =             note_array(form_date_const, item)  
        note_date =             note_array(note_date_const, item)  
    
        If trim(form_date) <> "" then
            'msgbox form_date
            If trim(note_date) = "" then
                'msgbox "note date is blank."
