...
file_selection_path = "T:\Eligibility Support\Restricted\QI - Quality Improvement\REPORTS\Recertification Accuracy Project\VA\VA COLA 2023.xlsx"
...
Dim UNEA_array()
ReDim UNEA_array (case_notes, 0)

'Sets constants for the array to make the script easier to read (and easier to code)'
Const case_number_const     = 0			'Each of the case numbers will be stored at this position'
Const memb_num_const   	    = 1
Const inc_type_const	    = 2
Const update_date_const     = 3
Const current_unea_const 	= 4
Const increased_unea_const  = 5
Const cola_disregard_const  = 6
Const case_status  	        = 7
Const case_notes   	        = 8
...
Do                                                            'Loops until there are no more cases in the Excel list
    ...
    member_number = objExcel.cells(excel_row, 5).value
    member_number = "0" & right(member_number, 2)

    current_unea = objExcel.cells(excel_row, 6).value
    current_unea = round(current_unea, 2)

    cola_disregard = objExcel.cells(excel_row, 7).value
    cola_disregard = round(cola_disregard, 2)

    increased_unea = objExcel.cells(excel_row, 8).value
    increased_unea = round(increased_unea, 2)


	'Adding client information to the array'
	ReDim Preserve UNEA_array(case_notes,       entry_record)	'This resizes the array based on the number of rows in the Excel File'
	UNEA_array (case_number_const,     entry_record) = MAXIS_case_number		'The client information is added to the array'
	UNEA_array (memb_num_const,        entry_record) = trim(member_number)
	UNEA_array (inc_type_const, 	   entry_record) = trim(objExcel.cells(excel_row, 4).value)
	UNEA_array (current_unea_const,    entry_record) = trim(current_unea)
	UNEA_array (cola_disregard_const,  entry_record) = trim(cola_disregard)
    UNEA_array (increased_unea_const,  entry_record) = trim(increased_unea)
...
For i = 0 to Ubound(UNEA_array, 2)
	...
	MAXIS_case_number	= UNEA_array (case_number_const, i)
	income_type 		= UNEA_array (inc_type_const, i)
    member_number       = UNEA_array (memb_num_const, i)
    prospective_amt     = UNEA_array (current_unea_const, i)
	     	...
	     	Do
                ...
                'msgbox income_type & vbcr & panel_income_type
	     		If income_type = panel_income_type then
                    ...
                    'If cdate(update_date) > cdate(first_of_month) then
                    '    'msgbox "updated this month"
                    '    UNEA_array(case_status, i) = "Error"
                    '    UNEA_array(case_notes, i) = "Case updated in 12/2022"	'Explanation for the rejected report'
                    '    income_panel_found = false
                    'Else
                        ...
                        Else
                            ...
                            'increase_amt = prospective_amt * .087 'VA (8.7%)
	                        ...
	                        If update_SNAP = true then
                                ...
                                EMWriteScreen UNEA_array(increased_unea_const, i), 8, 66
	                        ...
	                        IF update_HC = true then
	                        	...
	                        	EMWriteScreen UNEA_array(increased_unea_const, i), 9, 65
	                        ...
	                        EMWriteScreen UNEA_array(increased_unea_const, i), 13, 68		'Entering the income on the UNEA panel
	...
	IF income_panel_found = true then
	    ...
	    Call write_variable_in_CASE_NOTE("*" & note_header & " income update for 2023 COLA*")
        ...
        Call write_variable_in_case_note("---")
         Call write_variable_in_CASE_NOTE("* Verified 2022 amt: $" & UNEA_array(current_unea_const, i))
	    Call write_variable_in_CASE_NOTE("* COLA increased by $" & UNEA_array (cola_disregard_const, i) & "(" & increase_perc & ")")
	    Call write_variable_in_CASE_NOTE("* New UNEA amount: $" & UNEA_array(increased_unea_const, i))
