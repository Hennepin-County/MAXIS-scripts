...
Function read_detail_on_RPPH(curr_enrl_date, curr_hp_code, curr_cntrct_code_one, curr_cntrct_code_two, curr_cntrct_code_comb, curr_change_rsn_code, curr_disenrl_rsn_code, curr_hp_name, curr_chg_rsn_full, curr_disenrl_rsn_full)
	...
	If curr_hp_code = "A565813600" then curr_hp_name = "UCare - OLD CONTRACT"
    If curr_hp_code = "A390628400" then curr_hp_name = "UCare"
...
For each member in HH_member_array
	...
	Else
		...
		If hp_code = "A565813600" then MMIS_clients_array(current_plan, MMIS_clients_arrays) = "UCare - OLD CONTRACT"
...
For person = 0 to Ubound(MMIS_clients_array, 2)
    ...
    dlg_len = dlg_len + 20
Next
person_on_case_can_be_enrolled = False
...
BeginDialog Dialog1, 0, 0, 750, dlg_len, "Enrollment Information"
  ...
  For person = 0 to Ubound(MMIS_clients_array, 2)
    If MMIS_clients_array(enroll_allow, person) = True Then
		...
		person_on_case_can_be_enrolled = True
	Else
		Text 5, (x * 20) + 25, 95, 10, MMIS_clients_array(client_name, person)
		Text 100, (x * 20) + 25, 35, 10, MMIS_clients_array(client_pmi, person)
		Text 143, (x * 20) + 25, 95, 10, MMIS_clients_array(current_plan, person)
		Text 240, (x * 20) + 25, 300, 10, " - The script cannot process an enrollment for someone with this exclusion"
...
If person_on_case_can_be_enrolled = False Then script_end_procedure_with_error_report("The script will now end as there are no recipients that can be enrolled using the script.")
	...
	For member = 0 to Ubound(MMIS_clients_array, 2)
		If MMIS_clients_array(enroll_allow, member) = True Then
			...
			Do
				...
				If MMIS_clients_array(new_plan, member) = "UCare" then health_plan_code = "A390628400"  'code eff 01/2026
