...
Function send_to_OnBase	
	objWord.Application.ActivePrinter = "Hyland Software Virtual Printer"
	objDoc.PrintOut()
End Function
...
function save_to_desktop(save_name)
	desktop = CreateObject("WScript.Shell").specialfolders("Desktop")
	objDoc.SaveAs(desktop & "/" & save_name &".docx")
End function

function new_folder_on_desktop(folder_name)
	desktop = CreateObject("WScript.Shell").specialfolders("Desktop")
	CreateObject("Scripting.FileSystemObject").CreateFolder desktop & "/" & folder_name
End function

function save_to_desktop_in_folder(combined_name)
	desktop = CreateObject("WScript.Shell").specialfolders("Desktop")
	CreateObject("Scripting.FileSystemObject").CreateFolder desktop & "/" & combined_name	
	desktop = CreateObject("WScript.Shell").specialfolders("Desktop")
	objDoc.SaveAs(desktop & "/" & combined_name & "/" & combined_name &".docx")
end function

'DIALOGS----------------------------------------------------------------------------------------------------

BeginDialog single_pending_notice, 0, 0, 134, 302, "Single Pending Notice"
  EditBox 5, 15, 60, 12, client_first_name
  EditBox 70, 15, 60, 12, client_last_name
  EditBox 5, 40, 100, 12, client_address_line_1
  EditBox 110, 40, 20, 12, client_address_apt
  EditBox 5, 55, 125, 12, client_address_line_2
  EditBox 62, 71, 68, 12, client_application_date
  EditBox 62, 87, 68, 12, rfi_due_date
  EditBox 62, 103, 68, 12, client_case_number
  CheckBox 15, 131, 34, 10, "Income", rsn_income_type
  CheckBox 70, 131, 47, 10, "Deductions", rsn_deductions
  CheckBox 15, 144, 32, 10, "SSNID", rsn_ssnid
  CheckBox 70, 144, 53, 10, "Incarceration", rsn_incarceration
  CheckBox 15, 157, 74, 10, "Citizen Status Code", rsn_citizen_status_code
  CheckBox 15, 170, 62, 10, "Applied for SSN", rsn_applied_for_ssn
  CheckBox 15, 243, 70, 10, "Send to OnBase", setting_send_to_onbase
  CheckBox 15, 255, 70, 10, "Save to Desktop", setting_save_to_desktop
  CheckBox 15, 268, 63, 10, "Print", setting_print
  ButtonGroup ButtonPressed
    OkButton 78, 287, 20, 12
    CancelButton 101, 287, 30, 12
  Text 6, 5, 40, 8, "First Name"
  Text 71, 5, 40, 8, "Last Name"
  Text 6, 30, 50, 8, "Address"
  Text 111, 30, 12, 8, "Apt."
  Text 6, 73, 55, 8, "Application Date"
  Text 6, 89, 55, 8, "Due Date"
  Text 6, 105, 55, 8, "Case Number"
  GroupBox 5, 119, 125, 66, "Pending Reasons"
  GroupBox 5, 231, 125, 51, "Other Settings"
  Text 16, 192, 100, 33, "To send to OnBase you MUST have OnBase open and the Hyland Virtual Print Driver installed before you press OK."
EndDialog
...
'Do
	Dialog single_pending_notice
		If buttonpressed = 0 then stopscript
'Loop Until rfi_due_date <> ""
...
objSelection.TypeText client_address_line_1 & "          " & client_address_apt & Chr(11)
...
If rsn_excluded_income = 1 then
	...
	objSelection.TypeParagraph()
	End If
If rsn_citizen_status_code = 1 then
...
If rsn_deductions = 1 then
	...
	objSelection.TypeParagraph()
	End If
If rsn_projected_annual_income = 1 then
...
If setting_save_to_desktop = 1 then call save_to_desktop(client_last_name & ", " & client_first_name & " - " & client_case_number)
If setting_send_to_onbase = 1 then call send_to_OnBase
If setting_print = 1 then call print_word_doc
