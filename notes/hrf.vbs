...
function HRF_add_JOBS_to_variable(variable_name_for_JOBS)
	...
	Else
		...
		IF JOBS_panel_prosp_pay_amount <> "" THEN variable_name_for_JOBS = variable_name_for_JOBS & "- Prospective: $" & JOBS_panel_prosp_pay_amount & " total; "
...
function HRF_add_UNEA_to_variable(variable_name_for_UNEA)
	...
	Else
		...
		If UNEA_panel_prosp_pay_amount <> "" THEN variable_name_for_UNEA = variable_name_for_UNEA & "- Prospective: $" & UNEA_panel_prosp_pay_amount & " total; "
		'Leaving out HC income estimator if footer month is not Current month + 1
		current_month_for_hc_est = dateadd("m", "1", date)
		current_month_for_hc_est = datepart("m", current_month_for_hc_est)
		IF len(current_month_for_hc_est) = 1 THEN current_month_for_hc_est = "0" & current_month_for_hc_est
...
msgbox "programs_selected_list " & programs_selected_list


' If datediff("d", #04/01/2024#, #03/01/2025#) > 0 or (instr(programs_selected_list, "MFIP") = 0 and instr(programs_selected_list, "SNAP") = 0 and instr(programs_selected_list, "GA") = 0) Then
...
' Else
	...
	If instr(programs_selected_list, "MFIP") or instr(programs_selected_list, "SNAP") Then
		call HRF_autofill_editbox_from_MAXIS(HH_member_array, "UNEA", earned_income)
	Else
		call autofill_editbox_from_MAXIS(HH_member_array, "UNEA", earned_income)
	End If
