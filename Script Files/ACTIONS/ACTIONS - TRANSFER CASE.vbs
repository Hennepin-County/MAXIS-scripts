...
'----------THE SCRIPT----------
'connecting to MAXIS, checking for an active session & grabbing the footer month/year information 
EMConnect ""
...
IF XFERRadioGroup = 0 THEN
	...
	Do
		Do
			...
			cancel_confirmation
			If case_number = "" or IsNumeric(case_number) = FALSE then MsgBox "You must have a case number to continue."
			IF len(worker_to_transfer_to) <> 7 then Msgbox "Please include X102 in the worker number"
		Loop until (case_number <> "" or IsNumeric(case_number) = True AND len(worker_to_transfer_to) = 7)
