...
If CIT_check = "MEMI:CITIZENSHIP HAS BEEN VERIFIED THROUGH SSA" then run_DAIL_scrubber_script("C:\DHS-MAXIS-Scripts\Script Files\DAIL - citizenship verified.vbs")

'CS reports a new employer to the worker.
EMReadScreen CS_new_emp_check, 25, 6, 20
If CS_new_emp_check = "CS REPORTED: NEW EMPLOYER" then run_DAIL_scrubber_script("C:\DHS-MAXIS-Scripts\Script Files\DAIL - CS reported new employer.vbs")

'Child support messages.
EMReadScreen CSES_check, 4, 6, 6
If CSES_check = "CSES" then
  EMReadScreen CSES_DISB_check, 4, 6, 20
  If CSES_DISB_check = "DISB" then run_DAIL_scrubber_script("C:\DHS-MAXIS-Scripts\Script Files\DAIL - CSES processing.vbs")
End if

'Disability certification ends in 60 days.
EMReadScreen DISA_check, 58, 6, 20
If DISA_check = "DISABILITY IS ENDING IN 60 DAYS - REVIEW DISABILITY STATUS" then run_DAIL_scrubber_script("C:\DHS-MAXIS-Scripts\Script Files\DAIL - disa message.vbs")

'Client can receive an FMED deduction for SNAP.
EMReadScreen FMED_check, 59, 6, 20
If FMED_check = "MEMBER HAS TURNED 60 - NOTIFY ABOUT POSSIBLE FMED DEDUCTION" then run_DAIL_scrubber_script("C:\DHS-MAXIS-Scripts\Script Files\DAIL - FMED deduction.vbs")
...
If SCHL_check = "STUDENT INCOME HAS ENDED - REVIEW FS AND/OR HC RESULTS/APP" then run_DAIL_scrubber_script("C:\DHS-MAXIS-Scripts\Script Files\DAIL - student income.vbs")

'SSI info received by agency.
EMReadScreen SDX_check, 44, 6, 30
If SDX_check = "SDX INFORMATION HAS BEEN STORED - CHECK INFC" then run_DAIL_scrubber_script("C:\DHS-MAXIS-Scripts\Script Files\DAIL - SDX info has been stored.vbs")

'Random messages generated from an affiliated case.
EMReadScreen stat_check, 4, 6, 6
If stat_check = "FS  " or stat_check = "HC  " or stat_check = "GA  " or stat_check = "MSA " or stat_check = "STAT" then run_DAIL_scrubber_script("C:\DHS-MAXIS-Scripts\Script Files\DAIL - affiliated case lookup.vbs")

'SSA info received by agency.
EMReadScreen SVES_check, 31, 6, 30
If SVES_check = "TPQY RESPONSE RECEIVED FROM SSA" then run_DAIL_scrubber_script("C:\DHS-MAXIS-Scripts\Script Files\DAIL - TPQY response.vbs")
