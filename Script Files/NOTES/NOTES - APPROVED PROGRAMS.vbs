...
For each item in date_array
	...
	For row = 7 to 18
		...
		If versions_exist <> " " THEN
			...
			version_date = cdate(version_date)
			If version_date = date THEN
			...
			ElseIf abs(DateDiff("d", date, version_date)) < 60 Then
				EMReadScreen prog_to_check, 4, row, 22
			 	MsgBox "The eligibilty results for " & prog_to_check & " are not from today. Best practices say to run a case through background, approve, and case note all on the same day. The script will not case note the approval results."
			End If
		End If
	Next
Next
...
For all_elig_results = 0 to UBound (bene_amount_array,2)
	If bene_amount_array(0, all_elig_results) = "Food" AND snap_approved_check = checked Then
		...
		ELSE
			...
			End If
		End If
	MsgBox "SNAP " & bene_amount_array (3, all_elig_results)
