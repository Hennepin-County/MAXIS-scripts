...
HOLIDAYS_ARRAY = Array(#11/10/23#, #11/23/23#, #11/24/23#, #12/25/23#, #1/1/24#, #1/15/24#, #2/19/24#, #5/27/24#, #6/19/24#, #7/4/24#, #9/2/24#, #11/11/24#, #11/28/24#, #11/29/24#, #12/25/24#, #1/1/25#, #1/20/25#, #2/17/25#, #5/26/25#, #6/19/25#, #7/4/25#, #9/1/25#, #11/11/25#, #11/27/25#, #11/28/25#, #12/25/25#, #1/1/26#, #1/19/26#, #2/16/26#, #5/25/26#, #6/19/26#, #7/3/26#, #9/7/26#, #11/11/26#, #11/26/26#, #11/27/26#, #12/25/26#)
...
For Each objFile in colFiles																'looping through each file
	...
	If InStr(this_file_name, "interview-summary-answers-") <> 0 Then delete_this_file = True		'We want to delete files that say 'interview-answers-' as this is how the NOTES - Interview script creates the save your work doc
	If InStr(this_file_name, "interview_questions_") <> 0 		Then delete_this_file = True		'We want to delete files that say 'interview-answers-' as this is how the NOTES - Interview script creates the save your work doc
...
'~~~~~ orientation_notes: INTEGER - array index for notes about the MFIP orientation process.
            ...
            Do
                ...
                BeginDialog Dialog1, 0, 0, 551, 385, "MFIP Orientation for " & CAREGIVER_ARRAY(memb_name_const, caregiver)
                    ...
                    If MFIP_orientation_step = mf_step_ccap Then
                        ...
                        Text 40, 45, 120, 10, "- MFIP Childcare assistance"
...
'===== Keywords: MAXIS, ADMIN, NOTICE, Appointment, Application
	...
	Call write_variable_in_SPEC_MEMO("To complete a phone interview, call the Interview Line at")
	Call write_variable_in_SPEC_MEMO("612-348-9905 between 9:00am and 4:00pm Monday thru Friday.")
	Call write_variable_in_SPEC_MEMO(" ")
...
Function create_TIKL(TIKL_text, num_of_days, date_to_start, ten_day_adjust, TIKL_note_text)
    ...
    If cdate(TIKL_date) < date then
        msgbox "Unable to create TIKL, the TIKL date is a past date. Please manually track this case and action."   'fail-safe in case the TIKL date created is in the past. DAIL/WRIT does not allow past dates.
    Else
        If ten_day_adjust = True then
            ...
            IF TIKL_mo = "01" AND TIKL_yr = "26" THEN
                ten_day_cutoff = #01/21/2026#
            ELSEIF TIKL_mo = "02" AND TIKL_yr = "26" THEN
                ten_day_cutoff = #02/18/2026#
            ELSEIF TIKL_mo = "03" AND TIKL_yr = "26" THEN
                ten_day_cutoff = #03/19/2026#
            ELSEIF TIKL_mo = "04" AND TIKL_yr = "26" THEN
                ten_day_cutoff = #04/20/2026#
            ELSEIF TIKL_mo = "05" AND TIKL_yr = "26" THEN
                ten_day_cutoff = #05/20/2026#
            ELSEIF TIKL_mo = "06" AND TIKL_yr = "26" THEN
                ten_day_cutoff = #06/17/2026#
            ELSEIF TIKL_mo = "07" AND TIKL_yr = "26" THEN
                ten_day_cutoff = #07/21/2026#
            ELSEIF TIKL_mo = "08" AND TIKL_yr = "26" THEN
                ten_day_cutoff = #08/20/2026#
            ELSEIF TIKL_mo = "09" AND TIKL_yr = "26" THEN
                ten_day_cutoff = #09/18/2026#
...
'===== Keywords: SNAP, calculation, Income Test
	...
	ElseIf DateDiff("d", #10/1/2024#, month_to_review) >= 0 Then
			If hh_size = 1 Then fpg_130_percent = 1632
			If hh_size = 2 Then fpg_130_percent = 2215
			If hh_size = 3 Then fpg_130_percent = 2798
			If hh_size = 4 Then fpg_130_percent = 3380
			If hh_size = 5 Then fpg_130_percent = 3963
			If hh_size = 6 Then fpg_130_percent = 4546
			If hh_size = 7 Then fpg_130_percent = 5129
...
'===== Keywords: SNAP, EA, EGA, calculation, Income Test, FPG
    ...
    If program_determination = "EA" Then
        ...
        If (DateDiff("d", #4/1/2025#, date)) >= 0 Then
            ...
            If hh_size_variable = 4 Then fpg_200_percent = 5358
            If hh_size_variable = 5 Then fpg_200_percent = 6275
            If hh_size_variable = 6 Then fpg_200_percent = 7192
            If hh_size_variable = 7 Then fpg_200_percent = 8108
