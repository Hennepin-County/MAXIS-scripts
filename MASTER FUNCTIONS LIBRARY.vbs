...
'This part takes care of remaining navigation buttons, designed to go to a single panel.
    'APPL
    If ButtonPressed = APPL_button then call navigate_to_MAXIS_screen("APPL", "") 'Application (APPL)
    If ButtonPressed = MEMB_button then call navigate_to_MAXIS_screen("MEMB", "") 'Household Members (MEMB)
    If ButtonPressed = MEMI_button then call navigate_to_MAXIS_screen("MEMI", "") 'Additional Member Info (MEMI)
    If ButtonPressed = ADDR_button then call navigate_to_MAXIS_screen("ADDR", "") 'Address (ADDR)
    'DAIL
    ...
    'MONY
    If ButtonPressed = INQD_button then call navigate_to_MAXIS_screen("MONY", "INQD")
    If ButtonPressed = INQX_button then call navigate_to_MAXIS_screen("MONY", "INQX")
    If ButtonPressed = INQB_button then call navigate_to_MAXIS_screen("MONY", "INQB")
    If ButtonPressed = INQD_button then call navigate_to_MAXIS_screen("MONY", "INQD")
    'MFIPELIG
    ...
    'CCOL
    If ButtonPressed = CLIC_button then call navigate_to_MAXIS_screen("CCOL", "CLIC") 'Claim Inquiry- By Case (CUC)
    ...
    'If ButtonPressed = CLSM_button then call navigate_to_MAXIS_screen("CCOL", "CLSM") 'Claim Repayment Agreement (CLSM)
    'If ButtonPressed = CLSM_button then call navigate_to_MAXIS_screen("CCOL", "CLSM") 'Demand Letter Text- Claim (CLSM)
    ...
    'REPT
    If ButtonPressed = USER_button then call navigate_to_MAXIS_screen("REPT", "USER")  'MAXIS User Information (USER)
    ...
    If ButtonPressed = RPTS_button then call navigate_to_MAXIS_screen("REPT", "RPTS")  'County Program Totals (RPTS)
    If ButtonPressed = MFCM_button then call navigate_to_MAXIS_screen("REPT", "MFCM")  'MFIP Participant Case Management (MFCM)
    If ButtonPressed = WBCM_button then call navigate_to_MAXIS_screen("REPT", "WBCM")  'Work Benefit Case Mgmt (WBCM)
    ...
    'If ButtonPressed = GRMR_button then call navigate_to_MAXIS_screen("REPT", "GRMR")  'GRH Status Update (GRMR) ' GRMR IS NO LONGER VALID 05/09/22
    ...
    'INFC
    If ButtonPressed = BEER_button then call navigate_to_MAXIS_screen("INFC", "BEER")  'Beneficiary Earnings Exchange (BEER)
    If ButtonPressed = BNDX_button then call navigate_to_MAXIS_screen("INFC", "BNDX")  'Beneficiary Data Exchange (BNDX)
    ...
    If ButtonPressed = IEVP_button then call navigate_to_MAXIS_screen("INFC", "IEVP")  'Person Verification Menu (IEVP)
    If ButtonPressed = INTM_button then call navigate_to_MAXIS_screen("INFC", "INTM")  'PARIS Interstate Match (INTM)
    If ButtonPressed = MLAD_button then call navigate_to_MAXIS_screen("INFC", "MLAD")  'MIPAA LIS Appl Detail (MLAD)
    If ButtonPressed = SDXS_button then call navigate_to_MAXIS_screen("INFC", "SDXS")  'State Data Exchange Summary (SDXS)
    If ButtonPressed = SDXI_button then call navigate_to_MAXIS_screen("INFC", "SDXI")  'State Data Exchange-Income (SDXI)
    ...
    If ButtonPressed = UNVI_button then call navigate_to_MAXIS_screen("INFC", "UNVI")  'Unverified Unearned Income (UNVI)
    If ButtonPressed = UBEN_button then call navigate_to_MAXIS_screen("INFC", "UBEN")  'Job Service Unemployment (UBEN)
    If ButtonPressed = WF1M_button then call navigate_to_MAXIS_screen("INFC", "WF1M")  'WF1 Manual Referral (WF1M)
    If ButtonPressed = WAGE_button then call navigate_to_MAXIS_screen("INFC", "WAGE")  'Job Service Wage (WAGE)
