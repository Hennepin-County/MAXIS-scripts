...
CALL write_variable_in_CCOL_note_test("-----" & Match_month & " PARIS MATCH " & "(" & first_name &  ") CLEARED " & rez_status & "-----")
...
'formatting for multiple states
For item = 0 to Ubound(state_array, 2)
	CALL write_variable_in_CCOL_note_test("----- Match State: " & state_array(state_name, item) & " -----")
	CALL write_bullet_and_variable_in_CCOL_note_test("Match State Active Programs", state_array(progs, item))
	CALL write_bullet_and_variable_in_CCOL_note_test("Match State Contact Info", state_array(contact_info, item))
NEXT
CALL write_variable_in_CCOL_note_test ("-----")
CALL write_bullet_and_variable_in_CCOL_note_test("Client accessing benefits in other state", bene_other_state)
CALL write_bullet_and_variable_in_CCOL_note_test("Contacted other state", contact_other_state)
CALL write_bullet_and_variable_in_CCOL_note_test("Verification used to clear", pending_verifs)
CALL write_bullet_and_variable_in_CCOL_note_test("Resolution Status", resolution_status)
IF rez_status = "FR" THEN CALL write_variable_in_CCOL_note_test("Client has failed to cooperate with Paris Match - has not provided requested verifications showing they are living in MN. Client will need to provide this before the case is reopened ")
...
CALL write_variable_in_CCOL_note_test("----- ----- ----- ----- ----- ----- -----")
CALL write_variable_in_CCOL_note_test ("DEBT ESTABLISHMENT UNIT 612-348-4290 EXT 1-1-1")

script_end_procedure("Success PARIS match updated and copied to CCOL.")
