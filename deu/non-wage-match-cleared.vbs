''GATHERING STATS===========================================================================================
name_of_script = "NON-WAGE MATCH CLEARED.vbs"
start_time = timer
STATS_counter = 1
...
'		script_end_procedure("DAYS No pending IEVS match found. Please review IEVP.")
'	ELSE
...
'		'MsgBox end_month
'		IF trim(start_month) = "" THEN Found_match = False
'		IF (trim(end_month)  = "" and IEVS_type = "UBEN") THEN 'ensuring if it is a wage the match is a month
'			found_match = TRUE
'			EXIT DO
'		ELSE
'			month_difference = abs(end_month) - abs(start_month)
'			IF (IEVS_type = "BEER" and month_difference = 11) THEN  'ensuring that if it a beer that the match is a year'
'				found_match = TRUE
'				EXIT DO
'			END IF
'		END IF
...
'	row = row + 1
'LOOP UNTIL row = 17
'IF found_match <> TRUE THEN script_end_procedure("No pending IEVS match found. Please review IEVP.")
''---------------------------------------------------------------------IULA
