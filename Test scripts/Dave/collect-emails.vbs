...
name_of_script = "BULK - EMAILS FROM LIST.vbs"
start_time = timer
...
call changelog_update("1/15/26", "Initial version.", "Dave Courtright, Hennepin")
...
'Dialog1 = ""
'  BeginDialog Dialog1, 0, 0, 201, 65, "EMAILS from List"
...
'ELSEIF run_mode = "Excel File" THEN
    ...
    '>>>>>DLG for Excel mode<<<<<
    Dialog1 = ""
    BeginDialog Dialog1, 0, 0, 256, 135, "Excel Information"
...
'FOR email_case = 0 to ubound(case_number_array)
'	IF MAXIS_case_number <> "" THEN
'        'Checking PRIV status
'        Call navigate_to_MAXIS_screen_review_PRIV("STAT", "ADDR", is_this_priv)
'		If is_this_priv = True then
...
'            EMReadScreen email_address, 50, 19, 31
'            email_address = trim(email_address)
'            objExcel.Cells(email_case + 2, 1).Value = case_number_array(email_case)
'            objExcel.Cells(email_case + 2, 2).Value = email_address
'		END IF
'	END IF
'NEXT
...
IF privileged_array <> "" THEN
	...
	MsgBox "The script could not collect email for the following privileged cases..." & vbCr & privileged_array
END IF
