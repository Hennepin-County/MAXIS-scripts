    ...
    If SNAP_checkbox = checked OR cash_checkbox = checked Then
        ...
        If confirm_update_prog = 1 Then
            ...
            If prog_update_cash_checkbox = checked Then
                ...
                EMReadScreen cash_one_app, 4, 6, 33
                EMReadScreen cash_two_app, 4, 7, 33

                cash_one_app = replace(cash_one_app, " ", "/")
                cash_two_app = replace(cash_two_app, " ", "/")

                if cash_one_app = CAF_datestamp then prog_row = 6
                if cash_two_app = CAF_datestamp then prog_row = 7
...
If do_not_update_prog = 1 Then end_msg = "Success! CAF has been successfully noted. Please remember to run the Approved Programs, Closed Programs, or Denied Programs scripts if  results have been APP'd." & vbNewLine & vbNewLine & "It was selected that PROG would NOT be updated because " &no_update_reason
