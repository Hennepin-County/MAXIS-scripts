...
name_of_script = "BULK - FIND MAEPD INFO.vbs"
start_time = timer
...
call changelog_update("01/12/2018", "Initial version.", "Ilse Ferris, Hennepin County")
...
If restart_checkbox = 0 then
    ...
    objExcel.Cells(1, 4).Value = "NEXT REVW DT"
    objExcel.Cells(1, 5).Value = "MA-EPD"
    objExcel.Cells(1, 6).Value = "Wavier codes"
	objExcel.Cells(1, 7).Value = "Medi Part B Amt"
...
DO
	...
	EMReadScreen HC_code, 2, 13, 59
	If HC_code = "22" then objExcel.Cells(excel_row, 5).Value = HC_code
	EMReadScreen waiver_code, 1, 14, 59
	If waiver_code <> "_" then objExcel.Cells(excel_row, 6).Value = waiver_code

	Call navigate_to_MAXIS_screen("STAT", "MEDI")
	EMReadScreen PartB_code, 8, 7, 73
	If PartB_code <> "________" then objExcel.Cells(excel_row, 7).Value = replace(PartB_code, "_", "")
