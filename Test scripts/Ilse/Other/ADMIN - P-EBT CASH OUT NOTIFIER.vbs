...
file_selection_path = "C:\Users\ilfe001\OneDrive - Hennepin County\Desktop\P-EBT Phase 1 Hennepin Cash Out.xlsx"
...
For i = 0 to Ubound(Cases_array, 2)
    ...
    IF Cases_array(case_status, i) = "" then 
        ...
        'msgbox child_count & vbcr & child_text
        ...
        Call write_variable_in_SPEC_MEMO("We are aware that " & child_count & " " & child_text & " on your case has not received Pandemic (P)EBT benefits. The reason the " & child_text & " did not receive the P-EBT benefit is due to receiving Supplemental Security Income (SSI). Follow the steps below to apply for P-EBT benefits for the school age " & child_text & " affected.")
        Call write_variable_in_SPEC_MEMO("")
        Call write_variable_in_SPEC_MEMO("Complete the application by July 31, 2020.")
        Call write_variable_in_SPEC_MEMO("")
        Call write_variable_in_SPEC_MEMO("1. Go to this web address to apply for P-EBT: mn.p-ebt.org")
        Call write_variable_in_SPEC_MEMO("2. Use the P-EBT application to apply for only the " & child_text & " that receives SSI and did not get an automatic P-EBT benefit in May 2020.")
        Call write_variable_in_SPEC_MEMO("3. Answer NO to the question asking if you have an EBT card. You will not be able to add the SSI " & child_text & " to the P-EBT application if you answer YES.")
        Call write_variable_in_SPEC_MEMO("4. You will receive a Confirmation Number at the end of the P-EBT application. Write this number down. You will need it to activate a P-EBT benefit card that you will receive in the mail within 30 days.")
        Call write_variable_in_SPEC_MEMO("")
        ...
        If make_case_note = True then 
            ...
            Call write_variable_in_CASE_NOTE("**P-EBT Issue for SSI Cashout Case**")
            Call write_variable_in_CASE_NOTE("--")
            Call write_variable_in_CASE_NOTE("A SPEC/MEMO has been sent to outline the process to be used to apply online for the SSI " & child_text & " that did not receive P-EBT benefits automatically.") 
            Call write_variable_in_CASE_NOTE("* Number of children that need to apply online: " & child_count)
