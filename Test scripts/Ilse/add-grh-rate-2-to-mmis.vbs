...
BeginDialog case_number_dialog, 0, 0, 216, 170, "Add GRH Rate 2 to MMIS"
  ...
  Text 10, 95, 195, 25, "This script is to be used when a GRH Rate 2 case needs to be added into MMIS. Currently this script only supports adding a new agreement to MMIS."
...
If Update_MMIS = True then  	
	...
	Else
		...
		IF disa_start_date <> "__ __ ____" then disa_start_date = Replace(disa_start_date," ","/")		'cleans up DISA dates
		If disa_end_date <> "__ __ ____" then disa_end_date = Replace(disa_end_date," ","/")
        ...
        revw_end_date = dateadd("d", -1, next_revw_date) & "" 	'blank space added to make vorianble to make a string
    ...
    Else
    	...
    	Do 
            ...
            If current_faci = True then exit do 
        Loop until SSRT_current_panel = SSRT_total_check
    End if 
    ...
    If current_faci = True then     
        ...
        Else
            ...
            If faci_found = False then 
                msgbox "FACI panel could not be found for the SSRT Rate 2 facility."
                Update_MMIS = False
                script_end_procedure("FACI panel could not be found for the SSRT panel vendor. The script will now end.")
            Else 
                Update_MMIS = True 
            End if 
        End if     
...
If Update_MMIS = True then
    BeginDialog date_dialog, 0, 0, 261, 175, "Select the SSR start and end dates"
    CheckBox 75, 25, 65, 10, "disa_start_date", disa_start_date
    CheckBox 170, 25, 65, 10, "disa_end_date", disa_end_date
    CheckBox 75, 45, 65, 10, "revw_start_date", revw_start_date
    CheckBox 170, 45, 65, 10, "revw_end_date", revw_end_date
    CheckBox 75, 65, 65, 10, "SSRT_start_date", SSRT_start_date
    CheckBox 170, 65, 65, 10, "SSRT_end_date", SSRT_end_date
    EditBox 75, 85, 55, 15, custom_start_date
    EditBox 170, 85, 55, 15, custom_end_date
    EditBox 85, 110, 170, 15, custom_dates_explained
    EditBox 85, 135, 170, 15, other_notes
    EditBox 85, 155, 80, 15, Edit9
    ButtonGroup ButtonPressed
    OkButton 170, 155, 40, 15
    CancelButton 215, 155, 40, 15
    ...
    Text 5, 140, 75, 10, "Other SSR/GRH notes:"
    GroupBox 165, 10, 90, 95, "Select the SSR end date:"
    Text 20, 160, 60, 10, "Worker signature:"
    ...
    DO
        DO 
            ...
            DO
                ...
                cancel_confirmation
                If (disa_start_date = unchecked and revw_start_date = unchecked and SSRT_start_date = unchecked and trim(custom_start_date) = "") then err_msg = err_msg & vbCr & "Choose/enter at least one start date."		'mandatory field
                If (disa_end_date = unchecked and revw_end_date = unchecked and SSRT_end_date = unchecked and trim(custom_end_date) = "") then err_msg = err_msg & vbCr & "Choose/enter at least one end date."		'mandatory field
                
                If (disa_start_date = checked or revw_start_date = checked or SSRT_start_date = unchecked or trim(custom_start_date) <> "") then err_msg = err_msg & vbCr & "Choose/enter at least one start date."		'mandatory field
                If (disa_end_date = unchecked or revw_end_date = unchecked or SSRT_end_date = unchecked or trim(custom_end_date) = "") then err_msg = err_msg & vbCr & "Choose/enter at least one end date."		'mandatory field
                
                If (trim(custom_start_date) <> "" or trim(custom_end_date) <> "" AND trim(custom_dates_explained) = "" then err_msg = err_msg & vbNewLine & "* Enter the reason for the custom start and/or end dates."
                If trim(worker_signature) = "" THEN err_msg = err_msg & vbCr & "Enter your worker signature."
                IF err_msg <> "" THEN MsgBox "*** NOTICE!!! ***" & vbNewLine & err_msg & vbNewLine		'error message including instruction on what needs to be fixed from each mandatory field if incorrect
            LOOP UNTIL err_msg = ""									'loops until all errors are resolved
            
            
            'revaluing the variables for the start date
            If disa_start_date = checked then disa_start_date = start_date
            If disa_end_date = checked then disa_end_date = start_date
            If revw_start_date = checked then revw_start_date = start_date
            If custom_start_date = checked then custom_start_date = start_date
            'revaluing the variables for the end date
            If SSRT_start_date = checked then SSRT_start_date = end_date 
            If SSRT_end_date = checked then SSRT_end_date = end_date 
            If revw_end_date = checked then revw_end_date = end_date 
            If custom_end_date = checked then custom_end_date = end_date 
        
            
            
            If IsDate(start_date) = false THEN err_msg = err_msg & vbCr & "Select/enter a valid start date."		'mandatory field
            IF IsDate(end_date) = false THEN err_msg = err_msg & vbCr & "Select/enter a valid end date."		'mandatory field
