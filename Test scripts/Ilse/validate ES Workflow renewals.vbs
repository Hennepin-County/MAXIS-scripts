'STATS GATHERING----------------------------------------------------------------------------------------------------
name_of_script = "ADMIN - VALIDATE ES WORKFLOW RENEWALS.vbs"
start_time = timer
STATS_counter = 1                          'sets the stats counter at one
STATS_manualtime = 90                      'manual run time in seconds
STATS_denomination = "I"       			   'M is for each CASE
'END OF stats block==============================================================================================
...
call changelog_update("05/13/2024", "Initial version.", "Ilse Ferris, Hennepin County")
...
file_selection_path = "C:\Users\ilfe001\OneDrive - Hennepin County\Desktop/Renewals Validation ES Workflow 5-13-2024.xlsx"
...
Do
    ...
    review_code = trim(objExcel.cells(excel_row, 5).value)
    program_ID = trim(objExcel.cells(excel_row, 6).value)

    Call navigate_to_MAXIS_screen_review_PRIV("STAT", "REVW", is_this_PRIV)
    If is_this_PRIV = True then 
        objExcel.cells(excel_row, 12).value = "PRIV Case."
    Else 
        EMReadScreen basket_number, 7, 21, 21
        objExcel.cells(excel_row, 7).value = basket_number
	    ...
	    objExcel.Cells(excel_row, 8).value = suggested_population

        If program_ID = "CA" then 
            EMReadScreen review_status, 1, 7, 40
            EMReadScreen next_review, 8, 9, 37
            EMReadScreen last_review, 8, 11, 37
        ElseIf program_ID = "FS" then 
            EMReadScreen review_status, 1, 7, 60
            EMReadScreen next_review, 8, 9, 57
            EMReadScreen last_review, 8, 11, 57
        End if 

        objExcel.Cells(excel_row, 9).value = next_review
        objExcel.Cells(excel_row, 10).value = review_status 
        objExcel.Cells(excel_row, 11).value = last_review
        If review_code = review_status then 
            'msgbox next_review & vbcr & last_review
            If next_review = "06 01 24" or last_review = "06 01 24" then review_validation = True 
        End if 

        objExcel.Cells(excel_row, 12).value = review_validation
