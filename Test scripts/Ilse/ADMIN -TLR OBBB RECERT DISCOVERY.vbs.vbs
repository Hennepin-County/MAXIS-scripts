'STATS GATHERING----------------------------------------------------------------------------------------------------
name_of_script = "ADMIN -TLR OBBB RECERT DISCOVERY.vbs"
start_time = timer
...
call changelog_update("10/03/2025", "Initial version.", "Ilse Ferris, Hennepin County")
...
'----------------------------------------------------------------------------------------------------Determining the EATS Household
	...
	'----------------------------------------------------------------------------------------------------'PROG for application date
    CALL navigate_to_MAXIS_screen("STAT", "PROG")
	EmReadScreen app_date, 8, 10, 33
	app_date = replace(app_date, " ", "/")
    ObjExcel.Cells(excel_row, app_date_col).Value = app_date
    ...
    IF memb_found = True THEN
		If SNAP_status <> "INACTIVE" then
                ...
                If eats_HH_count > 1 then
                    ...
                    FOR EACH eats_pers IN eats_group_members
                        IF trim(eats_pers) <> "" THEN
                            ...
                            If cl-age >= 14 and memb_age <= 17 then child_14_to_17 = true
                            IF cl_age =< 17 THEN
				    			child_under_18 = True
		    	    		Else
            ...
            objExcel.Cells(excel_row, child_under_14_col).Value = child_under_14
            objExcel.Cells(excel_row, child_14_to_17_col).Value = child_14_to_17
			...
			age_50_thru_55 = False
            age_55_thru_64 = false
            ...
            ObjExcel.Cells(excel_row, SNAP_disa_col).Value = disa_status
            ...
            ObjExcel.Cells(excel_row, homeless_col).Value = homeless_code
            ObjExcel.Cells(excel_row, living_sit_col).Value = living_situation
		...
		EMReadScreen next_revw_mo, 2, 9, 64	'ER
		EMReadScreen next_revw_yr, 2, 9, 70  'ER
		Transmit
	...
	ObjExcel.Cells(excel_row, SNAP_ER_col).Value = next_SNAP_revw
...
file_selection_path = "C:\Users\ilfe001\OneDrive - Hennepin County\Assignments\" & CM_mo & "-20" & CM_yr & " OBBB ER Discovery.xlsx"

'column constants
case_number_col 	= 1		'Col A
pmi_col	            = 2		'Col B
memb_numb_col   	= 3		'Col C
age_col             = 4		'Col D
SNAP_status_col     = 5		'Col E
app_date_col        = 6		'Col F
SNAP_ER_col         = 7		'Col G
homeless_col		= 8		'Col H
living_sit_col 		= 9		'Col I
eats_HH_col			= 10	'Col J
child_under_14_col  = 11	'Col K
child_14_to_17_col  = 12	'Col L
tribal_col          = 13	'Col M
native_col          = 14	'Col N
SNAP_disa_col       = 15	'Col O
CM_wreg_col		   	= 16	'Col P
CM_abawd_col		= 17	'Col Q
best_WREG_col		= 18	'Col R
best_abawd_col		= 19	'Col S
verified_wreg_col 	= 20	'Col T
counted_months_col	= 21	'Col U
notes_col    	    = 22	'Col V
all_exemptions_col	= 23	'Col W
...
Do
    ...
    If is_this_priv = True then
        report_notes = report_notes & "Privliged case. "
    Else
        ...
        If county_code <> UCASE(worker_county_code) then
            report_notes = report_notes & "Out-of-county Case. "
        Else
            Call navigate_to_MAXIS_screen("STAT", "MEMB")
            Do
                ...
                If trim(memb_panel_PMI) = PMI_number then
                    ...
                    End if
					ObjExcel.Cells(excel_row, native_col).Value = native    'native American indicator
                    Exit do
                Else
                    transmit
