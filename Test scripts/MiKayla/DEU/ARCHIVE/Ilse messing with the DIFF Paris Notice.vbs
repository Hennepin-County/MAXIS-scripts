...
Do 
	...
	End if 
	msgbox state & vbcr & Match_State_Case_Number
	msgbox add_state
	row = row + 3
	If row = 19 then	
		PF8
...
For item = 0 to Ubound(state_array, 2)
    ...
    Do 
    	Do
			...
			row = row + 1 
			msgbox "match prog row: " & row & vbcr & Match_Prog
...
BeginDialog SEND_PARIS_DIFF_NOTICE_dialog, 0, 0, 376, (240 + (add_state * 80)), "Paris Match Send Difference Notice"
  ...
  For item = 0 to Ubound(state_array, 2)
   GroupBox 5, (55 + (add_state * 80)), 360, 70, "PARIS Match found in " & 	state_array(state_name, 	item)
   Text 10, (75+ (add_state * 80)), 225, 10, "Match State Case Number: " & 	state_array(match_case_num, item)
   Text 10, (90+ (add_state * 80)), 265, 10, "Match Active Programs: " & 	state_array(progs, 		 	item)
   Text 10, (105+ (add_state * 80)), 265, 10, "Match contact info: " & 		state_array(contact_info, 	item)
  Next
  
  Text 10, (140 + (add_state * 80)), 130 , 10, "Client accessing benefits in other state:"
  DropListBox 140, (135 + (add_state * 80)), 55 , 15, "Select One:"+chr(9)+"YES"+chr(9)+"NO", bene_other_state
  Text 70, (165 + (add_state * 80)), 65 , 10, "Contact Other State:  & Contact_other_state"
  DropListBox 140, (160 + (add_state * 80)), 55 , 15, "Select One:"+chr(9)+"YES"+chr(9)+"NO", Contact_other_state
  GroupBox 205, (130  + (add_state * 80)), 160 , 50, "Verification requested:"
  CheckBox 210, (150 + (add_state * 80)), 50 , 10, "Diff Notice", Diff_Notice_Checkbox
  CheckBox 290, (150  + (add_state * 80)), 80 , 10, "Shelter Verification", Shelter_Verf_CheckBox
  CheckBox 210, (165  + (add_state * 80)), 70 , 10, "Proof of Residency", Proof_Residency_checkbox
  CheckBox 290, (165 + (add_state * 80)), 75 , 10, "School Verification", Schl_verf_checkbox
  Text 5, (200 + (add_state * 80)), 45, 10, "Other notes:"
  EditBox 50, (195 + (add_state * 80)), 200, 15, Other_Notes
  ButtonGroup ButtonPressed
    OkButton 260, (195 + (add_state * 80)), 50, 15
    CancelButton 315, (195 + (add_state * 80)), 50, 15
EndDialog
