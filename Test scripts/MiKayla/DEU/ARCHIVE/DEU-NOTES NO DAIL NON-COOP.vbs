...
name_of_script = "DEU-ACTION-PARIS-MATCH.vbs"
start_time = timer
...
CALL changelog_update("09/27/2017", "Updates created to add options for sending difference notice and handling for resolution status", "MiKayla Handley, Hennepin County")
...
NEXT
'-------------------------------THIS IS WHERE I AM dialog or no dialog?'
IF notice_sent = "N" THEN
    DO
    	DO
    		...
    		IF clear_action_checkbox = CHECKED THEN EXIT DO
...
'----------------------------------------------------------------Defaulting checkboxes to being checked (per DEU instruction)
    ...
    BeginDialog PARIS_MATCH_CLEARED_dialog, 0, 0, 376, 235, "PARIS MATCH CLEARED"
    ...
    GroupBox 5, 55, 360, 70, "PARIS MATCH STATE:  "  &	state_array(state_name, 0)
    	Text 10, 90, 170, 10, "Match Active Programs: "   & state_array(progs, 0)
    	Text 10, 110, 170, 15, "Match contact info: "   &  state_array(contact_info, 0)
			Text 10, 75, 155, 10, "Match State Case Number: "   & state_array(match_case_num, 0)
			Text 185, 60, 110, 10, "2nd Match State: "   &  add_state
    	Text 185, 90, 185, 10, "2nd Match active programs: "   & add_active_programs
    	Text 185, 75, 110, 10, "2nd Match State Case Number: " & add_case_num
    	Text 185, 105, 175, 15, "2nd Match contact info: "  & add_contact_info
    	Text 60, 180, 60, 10, "Refferal to Fraud:  "
      ...
      DropListBox 120, 175, 55, 15, "Select One:"+chr(9)+"YES"+chr(9)+"NO"+chr(9)+"Undetermined", fraud_refferal
...
ELSEIF clear_action_checkbox = CHECKED or notice_sent = "Y" THEN
	...
	GroupBox 5, 55, 360, 70, "PARIS MATCH STATE:  "  &	state_array(state_name, 0)
	 	Text 10, 90, 170, 10, "Match Active Programs: "   & state_array(progs, 0)
	 	Text 10, 75, 155, 10, "Match State Case Number: "   & state_array(match_case_num, 0)
	 	Text 10, 105, 170, 15, "Match contact info: "   &  state_array(contact_info, 0)
	 	Text 185, 60, 110, 10, "2nd Match State: "   &  add_state
	 	Text 185, 90, 185, 10, "2nd Match active programs: "   & add_active_programs
	 	Text 185, 75, 110, 10, "2nd Match State Case Number: " & add_case_num
	 	Text 185, 105, 175, 15, "2nd Match contact info: "  & add_contact_info
