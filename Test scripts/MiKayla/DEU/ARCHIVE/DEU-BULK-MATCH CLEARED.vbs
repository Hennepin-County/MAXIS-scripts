'GATHERING STATS===========================================================================================
name_of_script = "DEU-BULK MATCH CLEARED.vbs"
start_time = timer
STATS_counter = 1
...
call changelog_update("11/30/2017", "Added handling for requested resolutions.", "MiKayla Handley, Hennepin County")
call changelog_update("11/14/2017", "Program information will be input into the Excel spreadsheet. This will not need to be added manually by staff completing the cases.", "Ilse Ferris, Hennepin County")
...
DO
	...
	resolution_status	    = objExcel.cells(excel_row, 8).value	'establishes cleared status for the match
	'cleaned up
		...
		Else
			...
			Else
				...
				IF OutOfCounty_error = "MATCH IS NOT"  then
					...
					IF resolution_status = "BC - Case Closed" 	THEN CALL write_value_and_transmit("Case closed. " & other_notes, 8, 6)   							'BC
					IF resolution_status = "BE - No Change" THEN CALL write_value_and_transmit("No change. " & other_notes, 8, 6) 									'BE
					IF resolution_status = "BE - Child" THEN CALL write_value_and_transmit("No change, minor child income excluded. " & other_notes, 8, 6) 			'BE - child
					IF resolution_status = "BN - Already known, no savings" THEN CALL write_value_and_transmit("Already known - No savings. " & other_notes, 8, 6) 	'BN
					IF resolution_status = "BO - Other" THEN CALL write_value_and_transmit("HC Claim entered. " & other_notes, 8, 6) 								'BO
					IF resolution_status = "BP - Wrong Person" THEN CALL write_value_and_transmit("Client name and wage earner name are different. " & other_notes, 8, 6) 	
					IF resolution_status = "CC - Claim Entered" THEN CALL write_value_and_transmit("Claim entered. ", 8, 6)
    ...
    If case_note_actions = True then		'Formatting for the case note
        ...
        If resolution_status = "BN" or resolution_status = "BE" then Call write_variable_in_CASE_NOTE("CLIENT REPORTED EARNINGS. INCOME IS IN STAT/JOBS AND BUDGETED.")
        If resolution_status <> "CC" then Call write_variable_in_CASE_NOTE("NO OVERPAYMENTS OR SAVINGS RELATED TO THIS MATCH.")
        call write_variable_in_CASE_NOTE("------ ----- ----- ----- -----")
