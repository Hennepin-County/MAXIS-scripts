...
'Now it will create a new JOBS panel for this case.
	...
	IF TIKL_checkbox = UNCHECKED THEN script_end_procedure("Success! MAXIS updated for new NDNH HIRE message, and a case note made. An Employment Verification and Verifications Request should now be sent. The job is at " & employer & ".")
    'Navigates to TIKL
	IF TIKL_checkbox = CHECKED THEN
    	...
    	script_end_procedure("Success! MAXIS updated for new HIRE message, a case note made, and a TIKL has been sent for 10 days from now. An Employment Verification and Verifications Request should now be sent. The job is at " & employer & ".")
	END IF
END IF
