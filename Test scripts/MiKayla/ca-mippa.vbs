...
MAXIS_row = 15 'this part should be a for next?' can we jsut do a cursor read for now?
...
IF current_panel_check = "MTCH" THEN  ("Ensure duplicate PMIs have been reported if found, APPL using oldest PMI")
...
IF current_panel_check = "MTCH" THEN
	DO
		...
		PERS_check = MsgBox("Multiple matches found. Ensure duplicate PMIs have been reported, APPL using oldest PMI." & vbNewLine & "Press YES to confirm this is the match you wish to act on." & vbNewLine & "For the next match, press NO." & vbNewLine & vbNewLine & _
		"   " & PMI_number, vbYesNoCancel, "Please confirm this match")
		...
		If PERS_check = vbNo THEN
			...
			TRANSMIT
		END IF
		If (PERS_check = vbNo AND current_panel_number = panel_number) then	script_end_procedure("Unable to find another IMIG. Please review the case, and run the script again if applicable.")
