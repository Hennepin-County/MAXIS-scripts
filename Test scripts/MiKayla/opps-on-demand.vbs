...
name_of_script = "ADMIN - OPPS ON DEMAND.vbs"
start_time = timer
...
IF remove_memo_action = TRUE THEN
	CALL navigate_to_MAXIS_screen ("SPEC", "MEMO")
		...
		ELSE
			'DO
			row = 7
			EMReadScreen previous_date, 2, row, 22
			IF previous_date = "18" THEN
				row = row + 1
				EMReadScreen current_date, 2, row, 22
				IF current_date = "19" THEN
					Call write_value_and_transmit("C", row, 16)
					EMReadScreen panel_check, 4, 8, 6
					IF panel_check = "Canc" THEN action_completed = TRUE
				END IF
        		IF panel_check <> "Canc" THEN
					MsgBox panel_check
