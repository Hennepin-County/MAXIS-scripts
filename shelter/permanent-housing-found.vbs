...
DO      
      ...
      If isNumeric(num_days) = False then err_msg = err_msg & vbNewLine & "* Enter number of days in shelter."
      If isNumeric(rent_needed) = False then err_msg = err_msg & vbNewLine & "* Enter rent amount needed to move in."
      If isNumeric(DD_needed) = False then err_msg = err_msg & vbNewLine & "* Enter DD amount needed to move in."
      If trim(rent_subsidy_paid) = "" then err_msg = err_msg & vbNewLine & "* Enter who is paying for the rental subsidy."
      If isNumeric(rent_subsidy) = False then err_msg = err_msg & vbNewLine & "* Enter subsidy amount."
      If trim(additional_funds) = "" then err_msg = err_msg & vbNewLine & "* Enter who is issuing the additional funds."
      If trim(LL_name) = "" then err_msg = err_msg & vbNewLine & "* Enter the Landlord's name."
      If trim(LL_ADDR) = "" then err_msg = err_msg & vbNewLine & "* Enter the Landlord's address."
      If trim(REW) = "" then err_msg = err_msg & vbNewLine & "* Enter Rapid Exit Worker's name."
      If IsDate(closed_servicepoint) = False then err_msg = err_msg & vbNewLine & "* Enter ServicePoint close out date in DD/MM/YY format."
