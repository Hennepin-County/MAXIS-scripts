...
IF PF11_actions = "MFIP New Spouse Income" then
    ...
    Do
    	Do
    		...
    		if ButtonPressed = 0 then StopScript
			IF dail_date = "" THEN err_msg = err_msg & vbNewLine & "* Please enter the date or marriage."
			IF worker_xnumber = "" then err_msg = err_msg & vbNewLine & "* Please enter the worker phone number."
    		IF err_msg <> "" THEN MsgBox "*** NOTICE!***" & vbNewLine & err_msg & vbNewLine
    	Loop until err_msg = ""
    	Call check_for_password(are_we_passworded_out)
    LOOP UNTIL check_for_password(are_we_passworded_out) = False
...
IF PF11_actions <> "Case note removal request" THEN
	...
	IF PF11_actions = "MFIP New Spouse Income" THEN CALL write_bullet_and_variable_in_CASE_NOTE("Marriage Date", dail_date)
